...
5. 按 `ctrl+e` 编辑配置文件；改完保存后，回到程序按 `ctrl+r` 重载配置。如需定位 OCR/颜色识别区域，请参考后文的[定位识别区域](#定位识别区域)部分。

> 初次运行时程序会根据你的屏幕分辨率生成推荐的识别区域配置（仅支持部分常见分辨率）。
...
# 每个按键操作包含一个动作按键 (key)、一个按住时间 (hold)、一个组合键间隔时间 (interval)、一个延迟时间 (delay) 和一个执行次数 (times)
# 动作按键 (key)：支持 'm', 'enter', 'up', 'down' 等，也支持用 '+' 连接组合键，如 'ctrl+c'
...
如果需要定位 OCR/颜色 识别区域，可使用 `RegionLocator.exe` 辅助工具。在程序中按 `ctrl+p` 打开命令面板，选择 `Open RegionLocator` 命令即可运行该工具。
