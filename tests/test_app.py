...
from gtaol_dre_helper.utils.config import CONFIG_FILE_NAME
from gtaol_dre_helper.utils.paths import get_runtime_resource_path


def test_css_path_points_to_packaged_stylesheet() -> None:
    # 验证应用样式文件路径始终绑定到 app.py 同目录下的 style.scss，便于源码运行和 PyInstaller 打包后都能找到资源。
...
def test_runtime_resource_paths_use_project_root_in_source_mode(monkeypatch) -> None:
    # 验证源码运行时，配置文件与 Tesseract 都会相对项目根目录定位，而不是依赖当前工作目录。
